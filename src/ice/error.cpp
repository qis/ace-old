...
public:
  ...
  std::string message(int ev) const override
  {
#if ICE_OS_WIN32
    std::wstring wstr;
    constexpr DWORD type = FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
      FORMAT_MESSAGE_IGNORE_INSERTS;
    constexpr DWORD lang = MAKELANGID(
      LANG_NEUTRAL, SUBLANG_NEUTRAL);  // MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US);
    const auto id = static_cast<DWORD>(ev);
    LPWSTR buffer = nullptr;
    if (const auto size = FormatMessageW(
          type, nullptr, id, lang, reinterpret_cast<LPWSTR>(&buffer), 0, nullptr))
    {
    ...
    if (!wstr.empty()) {
      ...
      auto size = WideCharToMultiByte(CP_UTF8, 0, src_data, src_size, nullptr, 0, nullptr, nullptr) +
        1;
      if (size > 0) {
        std::string str(static_cast<std::size_t>(size) + 1, '\0');
        size = WideCharToMultiByte(
          CP_UTF8, 0, src_data, src_size, str.data(), size, nullptr, nullptr);
        if (size > 0) {
